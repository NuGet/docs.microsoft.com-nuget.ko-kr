...
- `packages.config`에서 [PackageReference](../consume-packages/package-references-in-project-files.md)로 마이그레이션된 프로젝트의 경우에는 [msbuild -t:pack](../consume-packages/migrate-packages-config-to-package-reference.md#create-a-package-after-migration)를 사용합니다.
...
종속성 선언 및 버전 번호 지정에 대한 자세한 내용은 [packages.config](../reference/packages-config.md) 및 [패키지 버전화](../concepts/package-versioning.md)를 참조하세요. `dependency` 요소의 `include` 및 `exclude` 특성을 사용하여 패키지에 종속성의 자산을 직접 공개할 수도 있습니다. [.nuspec 참조 - 종속성](../reference/nuspec.md#dependencies)을 참조하세요.
...
- 비표준 버전 구성표를 사용하는 경우 [패키지 버전 관리](../concepts/package-versioning.md)에서 설명한 대로 NuGet 버전 관리 규칙을 고려해야 합니다.
