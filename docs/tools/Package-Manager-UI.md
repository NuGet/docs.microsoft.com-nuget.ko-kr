...
1. **Include 시험판** 및 **패키지 원본** 컨트롤 효과가 없습니다. 패키지를 제거 하는 경우.
...
1. <a name="implicit_reference"></a>일부 패키지에 대 한는 **업데이트** 단추가 비활성화 되 고 "암시적으로 참조 함을 SDK에서" 없다는 메시지가 표시 (또는 "AutoReferenced"). 메시지는 Microsoft.NETCore.App 또는 Microsoft.NETStandard.Library, 같은 패키지를 더 큰 프레임 워크 또는 SDK의 일부 이며 독립적으로 업데이트 되지 않아야 나타냅니다. (이러한 패키지도 내부적으로 표시 된 `<IsImplicitlyDefined>True</IsImplicitlyDefined>`.) 패키지를 업데이트 하려면 포함 하는 SDK 패키지 이름에서 유추 속해 있는, SDK를 업데이트 합니다. 예를 들어.NET Core SDK의 일부인 Microsoft.NETCore.App 같은 패키지, 따라서.NET Core SDK 설치를 업데이트 해야 합니다.
...
이 예제에서는 ClassLibrary1 프로젝트 ConsoleApp1 6.1.0 EntityFramework를 사용 하는 반면 6.2.0, EntityFramework를 사용 중인 합니다. 패키지 버전을 통합 하려면 다음을 수행 합니다.
