...
### <a name="update-package-improvements"></a>Update-Package 개선 사항
NuGet 1.4은 Update-Package 명령에 대 한 향상 된 기능을 제공 하므로 솔루션의 여러 프로젝트에서 동일한 버전의 패키지를 더 쉽게 유지할 수 있습니다. 예를 들어 패키지를 최신 버전으로 업그레이드 하는 경우 해당 패키지를 설치 하는 모든 프로젝트를 동일한 verision 업데이트 하는 것이 매우 일반적입니다.

`Update-Package`이제 명령을 사용 하 여 다음 작업을 쉽게 수행할 수 있습니다.
...
`-Safe`플래그는 주 버전 및 부 버전 구성 요소가 동일한 버전 으로만 업그레이드를 제한 합니다. 예를 들어 패키지의 버전 1.0.0을 설치 하 고 피드에서 1.0.1, 1.0.2 및 1.1 버전을 사용할 수 있는 경우이 `-Safe` 플래그는 패키지를 1.0.2로 업데이트 합니다. 플래그 없이 업그레이드 하면 `-Safe` 패키지가 최신 버전인 1.1로 업그레이드 됩니다.
...
기본적으로 `Update-Package` 패키지에서 명령을 실행 하거나 대화 상자를 사용 하 여 패키지를 업데이트 하면 피드의 최신 버전으로 업데이트 됩니다. 모든 패키지를 업데이트 하는 새로운 지원 기능을 사용 하 여 패키지를 특정 버전 범위로 잠그는 경우가 있을 수 있습니다. 예를 들어 응용 프로그램은 패키지의 버전 2. * 에서만 작동 하지만 3.0 이상에서는 작동 하지 않는 것을 미리 알 수 있습니다. 패키지를 실수로 업데이트 하는 것을 방지 하기 위해 NuGet 1.4에서는 `packages.config` 새 특성을 사용 하 여 파일을 직접 편집 하 여 패키지를 업그레이드할 수 있는 버전 범위를 제한 하는 지원을 추가 `allowedVersions` 합니다.

예를 들어 다음 예제에서는 `SomePackage` 버전 범위 2.0-3.0 (제외)로 패키지를 잠그는 방법을 보여 줍니다.
`allowedVersions`특성은 [버전 범위 형식을](../concepts/package-versioning.md#version-ranges)사용 하 여 값을 허용 합니다.
...
1.4에서는 특정 버전 범위에 대 한 패키지 잠금을 직접 편집 해야 합니다. NuGet 1.5에서는 명령을 통해이 범위를 배치 하는 지원을 추가할 계획 `Install-Package` 입니다.

### <a name="package-visualizer"></a>패키지 시각화 도우미
**도구**  ->  **라이브러리 패키지 관리자**  ->  **패키지 시각화 도우미** 메뉴 옵션을 통해 시작 된 새 패키지 시각화 도우미를 사용 하면 솔루션 내에서 모든 프로젝트 및 해당 패키지 종속성을 쉽게 시각화할 수 있습니다.
...
일부 NuGet 버전에서는 파일을 통해 표현 되는 새로운 기능을 소개 합니다 `.nuspec` .이 기능은 이전 버전의 nuget 대화 상자에서 인식 되지 않습니다.
...
_**참고**: 현재 세션에서 **온라인** 탭을 선택한 경우에만 확인이 수행 됩니다._
...
* **메뉴 이름 개선**: 대화 상자를 시작 하는 메뉴 옵션의 이름을 명확 하 게 변경 했습니다. 이제 메뉴 옵션이 **NuGet 패키지를 관리** 합니다.
* **최신 업데이트 날짜를 표시 하는 세부 정보 창**: NuGet 대화 상자는 **온라인** 또는 **업데이트** 탭을 선택한 경우 패키지에 대 한 세부 정보 창에 최신 업데이트 날짜를 표시 합니다.
* **표시 되는 태그 목록**: Nuget 대화 상자에 태그가 표시 됩니다.

### <a name="powershell-improvements"></a>Powershell 개선 사항
* **서명 된 powershell 스크립트**: NuGet에는 더 제한적인 환경에서 사용 하도록 설정 된 서명 된 powershell 스크립트가 포함 됩니다.
* **지원 요청**: 이제 패키지 관리자 콘솔에서 및 명령을 통해 프롬프트를 지원 `$host.ui.Prompt` `$host.ui.PromptForChoice` 합니다.
* 패키지 원본 **이름**: 플래그를 사용 하 여 패키지 원본을 지정할 때 패키지 원본 이름을 제공 하는 것이 지원 됩니다. `-Source`

### <a name="nugetexe-command-line-improvements"></a>nuget.exe 명령줄 기능 향상
* **NuGet 사용자 지정 명령**: MEF를 사용 하 여 사용자 지정 명령을 통해 nuget.exe 확장할 수 있습니다.
* **기호 패키지 만들기에 대 한 워크플로 간소화**: `-Symbols` 플래그는 일반 규칙 기반 폴더 구조에 적용 될 수 있습니다. 폴더 구조에는 소스와 파일만 포함 하 여 기호 패키지를 만들 수 있습니다 `.pdb` .
* **여러 소스 지정**: `NuGet install` 명령은 세미콜론을 구분 기호로 사용 하거나 여러 번 지정 하 여 여러 소스를 지정 하도록 지원 합니다 `-Source` .
* **프록시 인증 지원**: nuget 1.4는 인증을 필요로 하는 프록시 뒤에서 nuget을 사용할 때 사용자 자격 증명을 묻는 메시지를 표시 합니다.
* **nuget.exe 업데이트 주요 변경 내용**: `-Self` 이제 nuget.exe를 업데이트 하는 데 플래그가 필요 합니다. `nuget.exe Update` 이제는 파일에 대 한 경로를 사용 하 여 `packages.config` 패키지 업데이트를 시도 합니다. 이 업데이트는 * * 프로젝트 파일의 콘텐츠를 업데이트, 추가, 제거 하지 않습니다.
...
Nuget에는 nuget 패키지를 통해 간단한 [웹 기반 nuget 리포지토리](../hosting-packages/nuget-server.md) 를 호스트 하는 간단한 방법이 포함 되어 있습니다 `NuGet.Server` . NuGet 1.4을 사용 하면 경량 서버가 nuget.exe를 사용 하 여 패키지를 푸시하는 것을 지원 합니다.
최신 버전의는 `NuGet.Server` 이라는 새를 추가 합니다 `appSetting` `apiKey` . 키를 생략 하거나 비워 두면 피드에 패키지를 푸시할 수 없습니다. ApiKey를 값 (이상적으로는 강력한 암호)으로 설정 하면 nuget.exe를 사용 하 여 패키지를 푸시할 수 있습니다.
...
* [문제 1036](http://nuget.codeplex.com/workitem/1036): `NuGet Pack SomeProject.csproj` 빌드 후 이벤트에 추가 하면 무한 루프가 발생 하지 않습니다.
...
### <a name="bug-fixes"></a>버그 픽스
* [문제 1220](http://nuget.codeplex.com/workitem/1220): `install.ps1` / `uninstall.ps1` 프로젝트가 두 개 이상 있는 경우 모든 프로젝트에서 Update-Package 실행 되지 않습니다.
* [문제 1156](http://nuget.codeplex.com/workitem/1156): W2K3/XP에서 패키지 관리자 콘솔 중단 되었습니다 (Powershell 2가 설치 되지 않은 경우).
