...
* Visual Studio 2019에서 [필터링 된 솔루션](https://docs.microsoft.com/visualstudio/ide/filtered-solutions?view=vs-2019) 복원 지원- [#5820](https://github.com/NuGet/Home/issues/5820)
...
* Gen 1 자격 증명 공급자 플러그 인이 [gen 2](https://aka.ms/nuget-cross-platform-authentication-plugin) 로 대체 되었으며 곧 사용 되지 않을 예정입니다. [#7819](https://github.com/NuGet/Home/issues/7819)

## <a name="issues-fixed-in-this-release"></a>이번 릴리스에서 수정된 문제

**버그**

* NoOp 복원 작업을 수행 하는 경우에는 .obj 디렉터리에서 *. dgspec write를 사용 하지 않습니다.- [#7854](https://github.com/NuGet/Home/issues/7854)

* ~/.Era 내에서 만든 파일에 대 한 사용 권한이 너무 열려 [#7673](https://github.com/NuGet/Home/issues/7673)

* `dotnet list package --outdated`는 인증을 요구 하는 원본에서 작동 하지 않습니다. [#7605](https://github.com/NuGet/Home/issues/7605)

* VisualStudio는 패키지 참조가 없는 프로젝트에 대해를 호출 하면 기본값이 [#7005](https://github.com/NuGet/Home/issues/7005) PackageReference로 설정 된 경우에도 항상 app.config를 사용 합니다.

* PMC: 패키지를 제거할 수 없습니다 ("패키지를 찾을 수 없음"). - [#7268](https://github.com/NuGet/Home/issues/7268)
...
* plugins_cache 정상적으로 작동 하려면 더 짧은 경로가 필요 합니다. [#7770](https://github.com/NuGet/Home/issues/7770)

* 사용자가 특정 msbuild 버전을 요청 하지 않은 경우 msbuild 검색 경로를 선호 합니다. [#7786](https://github.com/NuGet/Home/issues/7786)

* `nuget.exe /?` 올바른 msbuild 버전이 나열 됩니다. [#7794](https://github.com/NuGet/Home/issues/7794)
...
* NuGet. 클라이언트의 경우 [#7629](https://github.com/NuGet/Home/issues/7629) 에는 &를 사용 합니다.
...
* NuGet 수정. 구성 문제- [#7326](https://github.com/NuGet/Home/issues/7326)
...
* 만든 nupkg에서 종속성 버전 범위를 유지 해야 `dotnet pack` (부동 버전이 사용 되는 경우에도)- [#7232](https://github.com/NuGet/Home/issues/7232)

* 사용자 수준 구성에 소스- [#7209](https://github.com/NuGet/Home/issues/7209) 있는 경우 인증 된 원본에서 `dotnet restore` 실패
...
* 로컬 구성에 지정 된 원본에 대 한 전역 구성의 자격 증명을 사용 하지 `dotnet add package`- [#6935](https://github.com/NuGet/Home/issues/6935)
...
* 인증 된 피드의 패키지를 사용 하 여 Linux에서 `dotnet restore`에 PAT를 사용할 수 없습니다. [#5651](https://github.com/NuGet/Home/issues/5651)

* 사용 하지 않도록 설정 된 컴퓨터 전체 피드로 인해 dotnet restore 실패 함- [#5410](https://github.com/NuGet/Home/issues/5410)

**Dcr**

* 나중에 "dotnet pack project. json"을 제거할 때 경고- [#7928](https://github.com/NuGet/Home/issues/7928)
 
...
* Nuget.exe- [#4538](https://github.com/NuGet/Home/issues/4538) 를 통해 원본 당 http 요청 수를 제한 합니다.
...
* Nuget은 NuGet에 자격 증명이 있는 경우에도 NTLM 인증을 선호 합니다. 자격 증명에 대 한 인증 유형을 필터링 하려면 추가 구성 옵션- [#5286](https://github.com/NuGet/Home/issues/5286)

* PackageReference (EmbedInteropTypes 기능 일치)를 사용 하도록 설정- [#2365](https://github.com/NuGet/Home/issues/2365)
...
* 보안 (dotnet 또는 mono를 통해 실행 하는 경우)-올바른 권한을 사용 하 여 obj 폴더를 만들어야 [#7908](https://github.com/NuGet/Home/issues/7908)

* 사용자 지정 nuget.exe 및 `PackageSignatureValidity: False`를 사용 하 여 mono/MacOS의 nuget.exe 복원이 실패 [#8011](https://github.com/NuGet/Home/issues/8011)
...
`RestoreAdditionalProjectSources`를 nothing으로 설정 하 여 대체 폴더를 사용 하지 않도록 설정 합니다.
