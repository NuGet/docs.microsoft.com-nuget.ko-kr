...
참고: `dependencyVersion` 및 `repositoryPath`는 `packages.config`를 사용하는 프로젝트에만 적용됩니다. `globalPackagesFolder`는 `project.json` 및 PackageReference 형식을 사용하는 프로젝트에만 적용됩니다.
