...
| (1.0,2.0) | 1.0 < < 2.0 x | 단독 정확한 범위 |
| [1.0,2.0) | 1.0 ≤ < 2.0 x | 혼합된 포괄 및 전용 시간 최소 최대 버전 |
| (1.0)    | 잘못된 | 잘못된 |

PackageReference를 사용 하는 경우 또는 `project.json` 에서는 와일드 카드 표기법을 사용 하 여 또한 참조 형식, NuGet 패키지로 \*, 주, 부, Patch 및 수의 시험판 접미사 부분에 대 한 합니다. 와일드 카드는 지원 되지 않습니다는 `packages.config` 형식입니다.
