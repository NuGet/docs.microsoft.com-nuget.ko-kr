...
이 API는 Visual Studio, nuget.exe 및 .NET CLI에서 nuget 클라이언트를 사용 하 여 [`dotnet restore`](/dotnet/core/tools/dotnet-restore?tabs=netcore2x) , Visual STUDIO UI에서 검색 및를 수행 하는 등의 작업을 수행 [`nuget.exe push`](../reference/cli-reference/cli-ref-push.md) 합니다.

참고 일부의 경우 nuget.org에 다른 패키지 원본에서 적용 하지 않는 추가 요구 사항이 있습니다. 이러한 차이점은 [Nuget.org 프로토콜](nuget-protocols.md)에서 설명 합니다.

사용 가능한 nuget.exe 버전을 간단히 열거 하 고 다운로드 하려면 끝점 [ 의tools.js](tools-json.md) 를 참조 하세요.
...
서비스 인덱스 스키마 버전은 `version` 서비스 인덱스의 속성으로 표시 됩니다. API는 버전 문자열의 주 버전 번호를 요구 `3` 합니다. 서비스 인덱스 스키마에 대 한 주요 변경 내용이 적용 되지 않으므로 버전 문자열의 부 버전이 증가 합니다.
...
[PackageBaseAddress](package-base-address-resource.md)               | yes      | 패키지 콘텐츠 가져오기 (. nupkg)
[PackageDetailsUriTemplate](package-details-template-resource.md)    | 아니요       | 패키지 정보 웹 페이지에 액세스 하기 위한 URL을 생성 합니다.
[PackagePublish](package-publish-resource.md)                        | yes      | 패키지를 푸시 및 삭제 하거나 목록에서 삭제 합니다.
[RegistrationsBaseUrl](registration-base-url-resource.md)            | yes      | 패키지 메타 데이터를 가져옵니다.
...
[SearchQueryService](search-query-service-resource.md)               | yes      | 키워드별로 패키지를 필터링 하 고 검색 합니다.
...
> [!Note]
> 원본을 구현 하지 않는 경우에는 `SearchAutocompleteService` 정상적으로 자동 완성 동작을 사용 하지 않도록 설정 해야 합니다. `ReportAbuseUriTemplate`을 구현 하지 않으면 공식 nuget 클라이언트는 nuget로 대체 합니다. 조직의 신고 신고 URL ( [Nuget/Home # 4924](https://github.com/NuGet/Home/issues/4924)에서 추적). 다른 클라이언트는 단순히 사용자에 게 신고 URL을 표시 하지 않도록 선택할 수 있습니다.
...
`PackageDisplayMetadataUriTemplate` 및은 `PackageVersionDisplayMetadataUriTemplate` 이 범주에 속합니다.

Thirdly V2 프로토콜과 긴밀 하 게 결합 된 리소스를 문서화 하지 않습니다. 이러한 리소스 자체는 의도적으로 문서화 되지 않습니다. `LegacyGallery`리소스가이 범주에 속합니다. 이 리소스를 통해 V3 서비스 인덱스는 해당 V2 원본 URL을 가리킬 수 있습니다. 이 리소스는를 지원 `nuget.exe list` 합니다.
...
HEAD   | 해당 요청에 대 한 응답 헤더를 페치합니다 `GET` .
...
`GET`API 끝점에 대 한 모든 요청은 HTTP 리디렉션 (301 또는 302)을 반환할 수 있습니다. 클라이언트는 헤더를 관찰 `Location` 하 고 후속를 실행 하 여 이러한 리디렉션을 정상적으로 처리 해야 합니다 `GET` . 특정 끝점과 관련 된 설명서는 리디렉션을 사용할 수 있는 경우를 명시적으로 호출 하지 않습니다.
...
X NuGet-클라이언트-버전   | **사용 되지 않음** 및 대체 `X-NuGet-Protocol-Version`
X NuGet-프로토콜-버전 | Nuget.org의 특정 경우에만 필요 합니다. [nuget.org 프로토콜](NuGet-Protocols.md) 을 참조 하세요.
X NuGet-세션 Id       | *선택 사항입니다* . NuGet 클라이언트 v 4.7 +는 동일한 NuGet 클라이언트 세션의 일부인 HTTP 요청을 식별 합니다.

`X-NuGet-Session-Id`에는의 단일 복원과 관련 된 모든 작업에 대해 단일 값이 있습니다 `PackageReference` . 자동 완성 및 복원과 같은 다른 시나리오의 경우 `packages.config` 코드를 팩터링 하는 방법으로 인해 여러 다른 세션 ID가 있을 수 있습니다.

## <a name="authentication"></a>인증

인증은 정의할 패키지 원본 구현에 남아 있습니다. Nuget.org의 경우 리소스에만 `PackagePublish` 특수 API 키 헤더를 통한 인증이 필요 합니다. 자세한 내용은 [ `PackagePublish` 리소스](package-publish-resource.md) 를 참조 하세요.
