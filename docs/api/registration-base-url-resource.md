...
NuGet V3 API를 사용 하 여 패키지 원본에서 사용할 수 있는 패키지에 대 한 메타 데이터를 가져올 수 있습니다. 이 메타 데이터는 [서비스 인덱스](service-index.md)에 `RegistrationsBaseUrl` 있는 리소스를 사용 하 여 페치할 수 있습니다.

아래 `RegistrationsBaseUrl` 에 있는 문서 컬렉션을 종종 "등록" 또는 "등록 blob" 이라고 합니다. 단일 `RegistrationsBaseUrl` 문서에 있는 문서 집합을 "등록 hive" 라고 합니다. 등록 하이브에는 패키지 원본에서 사용할 수 있는 모든 패키지에 대 한 모든 메타 데이터가 포함 됩니다.
...
이러한 등록은 압축 되지 않습니다 (묵시적 `Content-Encoding: identity`이 사용 됨을 의미 함). SemVer 2.0.0 패키지는이 hive에서 **제외** 됩니다.

### <a name="registrationsbaseurl340"></a>RegistrationsBaseUrl/3.4.0

이러한 등록은를 사용 `Content-Encoding: gzip`하 여 압축 됩니다. SemVer 2.0.0 패키지는이 hive에서 **제외** 됩니다.

### <a name="registrationsbaseurl360"></a>RegistrationsBaseUrl/3.6.0

이러한 등록은를 사용 `Content-Encoding: gzip`하 여 압축 됩니다. SemVer 2.0.0 패키지는이 하이브에 **포함** 됩니다.
...
등록 인덱스의 URL은 예측 가능 하며 클라이언트에서 패키지 ID를 제공 하 고 서비스 인덱스에서 등록 리소스의 `@id` 값을 결정할 수 있습니다. 등록 페이지 및 리프에 대 한 Url은 등록 인덱스를 검사 하 여 검색 됩니다.
...
Nuget.org에서 사용 하는 추론은 다음과 같습니다. 128 이상의 패키지 버전이 있는 경우에는 64 크기의 페이지로 리프를 나눕니다. 128 버전 보다 작은 경우 인라인 모두 등록 인덱스에 그대로 둡니다.
...
count  | integer          | 예      | 페이지에 있는 등록의 수입니다.
품목이  | 개체의 배열 | no       | 등록의 배열 및 연결 메타 데이터
작을  | string           | 예      | 페이지에서 가장 낮은 SemVer 2.0.0 버전 (포함)
부모(parent) | string           | no       | 등록 인덱스의 URL입니다.
위의  | string           | 예      | 페이지의 최고 SemVer 2.0.0 버전 (포함)

Page 개체 `upper` 의 및범위는특정페이지버전에대한메타데이터가필요한경우에유용합니다.`lower`
이러한 범위는 필요한 유일한 등록 페이지를 인출 하는 데 사용할 수 있습니다. 버전 문자열은 [NuGet의 버전 규칙](../concepts/package-versioning.md)을 따릅니다. 버전 문자열은 정규화 되며 빌드 메타 데이터를 포함 하지 않습니다. NuGet 에코 시스템의 모든 버전과 마찬가지로 버전 문자열의 비교는 [SemVer 2.0.0의 버전 우선 순위 규칙](http://semver.org/spec/v2.0.0.html#spec-item-11)을 사용 하 여 구현 됩니다.

속성 `parent` 은 등록 페이지 개체 `items` 에 속성이 있는 경우에만 표시 됩니다.

등록 페이지 개체에 `@id` 속성이없는경우에지정된URL을사용하여개별패키지버전에대한메타데이터를가져와야합니다.`items` 경우 `items` 에 따라 배열은 페이지 개체에서 최적화로 제외 됩니다. 단일 패키지 ID의 버전 수가 매우 큰 경우 등록 인덱스 문서는 특정 버전 또는 작은 버전의 특정 버전에 대해서만 관심이 있는 클라이언트에 대해 처리 하는 데 큰 부담이 됩니다.

속성이 있는 `items` `@id` 경우 모든 페이지 데이터가 이미 `items` 속성에 인라인 되어 있으므로 속성을 사용할 필요가 없습니다.

Page 개체 `items` 배열의 각 항목은 등록 리프 및 연결 된 메타 데이터를 나타내는 JSON 개체입니다.
...
catalogEntry   | 개체(object) | 예      | 패키지 메타 데이터를 포함 하는 카탈로그 항목입니다.
...
등록 리프 개체의 속성에는다음과같은속성이있습니다.`catalogEntry`
...
중단              | 개체(object)                     | no       | 패키지와 연결 된 사용 중단
description              | string                     | no       | 
iconUrl                  | string                     | no       | 
id                       | string                     | 예      | 패키지의 ID입니다.
licenseUrl               | string                     | no       |
licenseExpression        | string                     | no       | 
나열                   | boolean                    | no       | 없는 경우 나열 된 것으로 간주 해야 합니다.
minClientVersion         | string                     | no       | 
projectUrl               | string                     | no       | 
게시할지                | string                     | no       | 패키지가 게시 되었을 때의 ISO 8601 타임 스탬프를 포함 하는 문자열입니다.
...
`dependencyGroups` 속성은 패키지의 종속성을 나타내는 개체의 배열입니다. 대상 프레임 워크를 기준으로 그룹화 됩니다. 패키지에 종속성 `dependencyGroups` 이 없거나, 속성이 없거나, 빈 배열 `dependencies` 이거나, 모든 그룹의 속성이 비어 있거나 누락 된 경우

`licenseExpression` 속성의 값은 [NuGet 라이선스 식 구문을](https://docs.microsoft.com/en-us/nuget/reference/nuspec#license)준수 합니다.
...
문자열 `targetFramework` 은 nuget의 .net 라이브러리 [nuget. 프레임 워크](https://www.nuget.org/packages/NuGet.Frameworks/)에서 구현 된 형식을 사용 합니다. 을 지정 `targetFramework` 하지 않으면 종속성 그룹이 모든 대상 프레임 워크에 적용 됩니다.
...
range        | 개체(object) | no       | 종속성의 허용 되는 [버전 범위](../concepts/package-versioning.md#version-ranges-and-wildcards) 입니다.
등록 | string | no       | 이 종속성에 대 한 등록 인덱스의 URL입니다.

속성이 제외 되거나 빈 문자열인 경우 클라이언트는 기본적으로 버전 범위 `(, )`를 지정 해야 합니다. `range` 즉, 종속성의 모든 버전을 사용할 수 있습니다.
...
alternatePackage | 개체(object)           | no       | 대신 사용 해야 하는 패키지 종속성

속성 `reasons` 은 하나 이상의 문자열을 포함 해야 하며 다음 표의 문자열만 포함 해야 합니다.
...
속성에 `reasons` 알려진 집합이 아닌 문자열이 포함 된 경우 해당 문자열을 무시 해야 합니다. 문자열은 대/소문자를 구분 하지 `legacy` 않으므로와 `Legacy`동일 하 게 처리 되어야 합니다. 배열에는 순서 제한이 없으므로 임의의 순서로 문자열을 정렬할 수 있습니다. 또한 속성에 알려진 집합이 아닌 문자열만 포함 된 경우에는 "Other" 문자열만 포함 된 것으로 처리 해야 합니다.
...
등록 페이지에 등록 나뭇잎이 포함 되어 있습니다. 등록 페이지를 인출 하는 URL은 위에서 언급 한 `@id` [등록 페이지 개체](#registration-page-object) 의 속성에 의해 결정 됩니다.

배열이 등록 인덱스에 제공 되지 않는 경우 `@id` 값의 HTTP GET 요청은 개체가 루트로 포함 된 JSON 문서를 반환 합니다. `items` 개체에는 다음과 같은 속성이 있습니다.
...
등록 리프를 인출 하는 URL은 등록 인덱스 또는 `@id` 등록 페이지에 있는 등록 리프 개체의 속성에서 가져옵니다.
...
> [!Note]
> Nuget.org에서 패키지를 `published` 나열 하지 않으면 값이 연도 1900로 설정 됩니다.
