...
*NuGet 4.0 이상을 사용하는 .NET Core 프로젝트에서 복수 대상 지정에 대한 자세한 내용은 [MSBuild 대상으로 NuGet 압축 및 복원](../schema/msbuild-targets.md)을 참조하세요.*
...
- [프레임워크 버전 폴더 구조](#framework-version-folder-structure)
- [프로젝트에서 어셈블리 버전 및 대상 프레임워크 일치](#matching-assembly-versions-and-the-target-framework-in-a-project)
- [프레임워크 버전에 따라 어셈블리 그룹화](#grouping-assemblies-by-framework-version)
- [사용할 NuGet 대상 결정](#determining-which-nuget-target-to-use)
- [콘텐츠 파일 및 PowerShell 스크립트](#content-files-and-powershell-scripts)
...
지원되는 이름의 전체 목록은 [대상 프레임워크 참조](../schema/target-frameworks.md#supported-frameworks)를 참조하세요.
...
> [!Warning]
> 변경할 수 있는 콘텐츠 파일 및 스크립트 실행은 `packages.config` 형식에서만 제공됩니다. `project.json` 및 PackagesReference 형식을 사용할 때 제공되지 않고 새 패키지에서 사용하지 말아야 합니다.
