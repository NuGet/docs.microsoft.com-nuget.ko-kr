...
원본에 대한 사용자 이름과 암호를 저장하며, 일반적으로 `-username`와 함께 `-password` 및 `nuget sources` 스위치로 지정됩니다. 또한 `-storepasswordincleartext` 옵션을 사용하지 않는 한 암호는 기본적으로 암호화됩니다.
...
| 키 | 값 |
| --- | --- |
| format | 기본 패키지 관리 형식을 나타내는 부울입니다. `1`경우 format은 PackageReference입니다. `0`경우 format은 *패키지 .config*입니다. |
| disabled | 첫 번째 패키지를 설치할 때 기본 패키지 형식을 선택 하 라는 메시지를 표시할지 여부를 나타내는 부울입니다. `False`는 프롬프트를 숨깁니다. |
...
Windows 스타일 환경 변수를 사용 해야 합니다 (시작 및 종료%). Mac/Linux 에서도 마찬가지입니다. 구성 파일에 `$HOME/NuGetRepository` 하는 것은 확인 되지 않습니다. Mac/Linux에서 `%HOME%\NuGetRepository` 값이 `/home/myStuff/NuGetRepository`으로 확인 됩니다.

환경 변수가 없으면 NuGet에서 구성 파일의 리터럴 값을 사용합니다.

## <a name="example-config-file"></a>config 파일 예제

다음은 선택적 항목을 비롯 한 다양 한 설정을 보여 주는 예제 `nuget.config` 파일입니다.
