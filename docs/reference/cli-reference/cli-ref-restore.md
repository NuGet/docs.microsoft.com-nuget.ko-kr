...
@No__t_0 폴더에서 누락 된 패키지를 다운로드 하 여 설치 합니다. NuGet 4.0 + 및 PackageReference 형식과 함께 사용 하는 경우 필요한 경우 `obj` 폴더에 `<project>.nuget.props` 파일을 생성 합니다. 파일은 소스 제어에서 생략할 수 있습니다.
...
| 일부만 | 사용자 입력 또는 확인에 대 한 프롬프트를 표시 하지 않습니다. |
| OutputDirectory | 패키지가 설치 되는 폴더를 지정 합니다. 폴더를 지정 하지 않으면 현재 폴더가 사용 됩니다. @No__t_1 또는 `SolutionDirectory`를 사용 하지 않는 한 `packages.config` 파일을 사용 하 여 복원할 때 필요 합니다.|
| PackageSaveMode | 패키지 설치 후 저장할 파일 유형을 지정 합니다. `nuspec`, `nupkg` 또는 `nuspec;nupkg` 중 하나입니다. |
| PackagesDirectory | `OutputDirectory`와 동일합니다. @No__t_1 또는 `SolutionDirectory`를 사용 하지 않는 한 `packages.config` 파일을 사용 하 여 복원할 때 필요 합니다. |
| Project2ProjectTimeOut | 프로젝트 간 참조를 확인 하는 제한 시간 (초)입니다. |
| 폴더 | *(4.0 이상)* UWP 및 .NET Core 프로젝트에 대 한 모든 참조 프로젝트를 복원 합니다. @No__t_0를 사용 하는 프로젝트에는 적용 되지 않습니다. |
| RequireConsent | 패키지를 다운로드 하 고 설치 하기 전에 패키지 복원이 사용 되는지 확인 합니다. 자세한 내용은 [패키지 복원](../../consume-packages/package-restore.md)을 참조 하세요. |
| 솔루션 디렉터리 | 솔루션 폴더를 지정 합니다. 솔루션에 대 한 패키지를 복원 하는 경우 유효 하지 않습니다. @No__t_1 또는 `OutputDirectory`를 사용 하지 않는 한 `packages.config` 파일을 사용 하 여 복원할 때 필요 합니다. |
| 소스 | 복원에 사용할 패키지 원본 (Url)의 목록을 지정 합니다. 생략 하는 경우 명령은 구성 파일에 제공 된 소스를 사용 합니다. [NuGet 동작 구성](../../consume-packages/configuring-nuget-behavior.md)을 참조 하세요. 세미콜론을 사용 하 여 목록 항목을 구분 합니다. |
| 설정 | PackageReference 기반 프로젝트에서 마지막 복원이 성공한 경우에도 모든 종속성이 확인 되도록 합니다. 이 플래그를 지정 하는 것은 `project.assets.json` 파일을 삭제 하는 것과 비슷합니다. 이는 http 캐시를 우회 하지 않습니다. |
...
2. 다음 우선 순위 순서를 사용 하 여 패키지 폴더를 결정 합니다. 이러한 폴더를 찾을 수 없는 경우 NuGet에서 오류를 제공 합니다.
    ...
    - @No__t_0 지정 된 폴더입니다.
    - @No__t_0 값 `Nuget.Config`
    - @No__t_0 지정 된 폴더
    - `$(SolutionDir)\packages`

3. 솔루션에 대 한 패키지를 복원할 때 NuGet은 다음 작업을 수행 합니다.
    - 솔루션 파일을 로드 합니다.
    - @No__t_0에 나열 된 솔루션 수준 패키지를 `packages` 폴더에 복원 합니다.
    - @No__t_0에 나열 된 패키지를 `packages` 폴더에 복원 합니다. 지정 된 각 패키지에 대해 `-DisableParallelProcessing`를 지정 하지 않으면 패키지를 병렬로 복원 합니다.
