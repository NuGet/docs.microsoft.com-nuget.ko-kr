...
사용 되는 경우 `packages.config`는 일반적으로 프로젝트 루트에 있습니다. 첫 번째 NuGet 작업이 실행 될 때 자동으로 생성 되지만 `nuget restore`와 같은 명령을 실행 하기 전에 수동으로 만들 수도 있습니다.

[PackageReference](../consume-packages/Package-References-in-Project-Files.md) 를 사용 하는 프로젝트는 `packages.config`을 사용 하지 않습니다.

## <a name="schema"></a>스키마

스키마는 단순합니다. 다음과 같은 표준 XML 헤더는 각 참조에 하나씩 하나 이상의 `<packages>` 요소가 포함된 단일 `<package>` 노드입니다. 각 `<package>` 요소에는 다음과 같은 특성이 있을 수 있습니다.

| attribute | 필수 | Description |
| --- | --- | --- |
| id | yes | Newtonsoft.json 또는 Microsoft.AspNet.Mvc와 같은 패키지의 식별자입니다. | 
| 버전 | yes | 3.1.1 또는 4.2.5.11-beta 등 설치할 정확한 버전의 패키지입니다. 버전 문자열에는 3개 이상의 번호가 있어야 합니다. 네 번째는 시험판 접미사로써 선택 사항입니다. 범위는 허용되지 않습니다. | 
...
다음과 같은 `packages.config`는 9개의 패키지를 참조하지만 `Microsoft.Net.Compilers` 특성 때문에 사용 중인 패키지를 빌드할 때 `developmentDependency`가 포함되지 않습니다. 또한 Newtonsoft.Json에 대한 참조는 8.x 및 9.x 버전에 대한 업데이트를 제한합니다.
