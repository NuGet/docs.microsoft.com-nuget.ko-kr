...
- <span data-ttu-id="3e0a0-117">`.nuspec`를 `nuget.exe pack` 사용 하는 비 SDK 스타일 프로젝트의 경우를 사용 `packages.config` 합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-117">Use `.nuspec` with `nuget.exe pack` for non-SDK-style projects that use `packages.config`.</span></span>
...
#### <a name="version"></a><span data-ttu-id="3e0a0-142">버전</span><span class="sxs-lookup"><span data-stu-id="3e0a0-142">version</span></span>
...
#### <a name="projecturl"></a><span data-ttu-id="3e0a0-157">projectUrl</span><span class="sxs-lookup"><span data-stu-id="3e0a0-157">projectUrl</span></span>
...
[<span data-ttu-id="3e0a0-190">패키지 아이콘 nuspec 샘플.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-190">Package Icon nuspec sample.</span></span>](https://github.com/NuGet/Samples/tree/main/PackageIconNuspecExample)
...
#### <a name="requirelicenseacceptance"></a><span data-ttu-id="3e0a0-201">requireLicenseAcceptance</span><span class="sxs-lookup"><span data-stu-id="3e0a0-201">requireLicenseAcceptance</span></span>
...
#### <a name="copyright"></a><span data-ttu-id="3e0a0-216">저작권</span><span class="sxs-lookup"><span data-stu-id="3e0a0-216">copyright</span></span>
...
<span data-ttu-id="3e0a0-233">예를 들어 다음과 같은 가치를 제공해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-233">For example:</span></span>
```xml
<?xml version="1.0"?>
<package xmlns="http://schemas.microsoft.com/packaging/2010/07/nuspec.xsd">
    <metadata>
...
#### <a name="frameworkassemblies"></a><span data-ttu-id="3e0a0-248">frameworkAssemblies</span><span class="sxs-lookup"><span data-stu-id="3e0a0-248">frameworkAssemblies</span></span>
...
### <a name="metadata-attributes"></a><span data-ttu-id="3e0a0-265">메타 데이터 특성</span><span class="sxs-lookup"><span data-stu-id="3e0a0-265">metadata attributes</span></span>
...
<span data-ttu-id="3e0a0-274">명령줄에서 `nuget pack -properties <name>=<value>;<name>=<value>`를 사용하여 토큰 값을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-274">On the command line, you specify token values with `nuget pack -properties <name>=<value>;<name>=<value>`.</span></span> <span data-ttu-id="3e0a0-275">예를 들어 `.nuspec`에서 `$owners$` 및 `$desc$`와 같은 토큰을 사용하고 다음과 같이 압축 시간에 값을 제공할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-275">For example, you can use a token such as `$owners$` and `$desc$` in the `.nuspec` and provide the values at packing time as follows:</span></span>
...
| <span data-ttu-id="3e0a0-301">**$copyright $**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-301">**$copyright$**</span></span> | <span data-ttu-id="3e0a0-302">AssemblyInfo</span><span class="sxs-lookup"><span data-stu-id="3e0a0-302">AssemblyInfo</span></span> | <span data-ttu-id="3e0a0-303">AssemblyCopyright</span><span class="sxs-lookup"><span data-stu-id="3e0a0-303">AssemblyCopyright</span></span> |
...
### <a name="dependency-groups"></a><span data-ttu-id="3e0a0-352">종속성 그룹</span><span class="sxs-lookup"><span data-stu-id="3e0a0-352">Dependency groups</span></span>
...
> [!Important]
> <span data-ttu-id="3e0a0-393">패키지가 프로젝트에 설치되면 어셈블리 참조가 지역화된 위성 어셈블리로 간주되므로 NuGet은 `.resources.dll`이라는 DLL을 *제외한* 패키지의 DLL에 해당 어셈블리 참조를 자동으로 추가합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-393">When a package is installed into a project, NuGet automatically adds assembly references to the package's DLLs, *excluding* those that are named `.resources.dll` because they are assumed to be localized satellite assemblies.</span></span> <span data-ttu-id="3e0a0-394">이러한 이유로 다른 경우에 필수 패키지 코드가 포함되는 파일에는 `.resources.dll`을 사용하지 마세요.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-394">For this reason, avoid using `.resources.dll` for files that otherwise contain essential package code.</span></span>

<span data-ttu-id="3e0a0-395">이러한 자동 동작을 무시하고 패키지에 포함되는 파일을 명시적으로 제어하려면 `<files>` 요소를 `<package>`의 자식(및 `<metadata>`의 형제)으로 배치하고 각 파일을 별도의 `<file>` 요소로 식별합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-395">To bypass this automatic behavior and explicitly control which files are included in a package, place a `<files>` element as a child of `<package>` (and a sibling of `<metadata>`), identifying each file with a separate `<file>` element.</span></span> <span data-ttu-id="3e0a0-396">예를 들어 다음과 같은 가치를 제공해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-396">For example:</span></span>
...
<span data-ttu-id="3e0a0-397">NuGet 2.x 및 이전 버전과 `packages.config`를 사용하는 프로젝트의 경우 `<files>` 요소는 패키지를 설치할 때 변경할 수 없는 콘텐츠 파일을 포함하는 데에도 사용됩니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-397">With NuGet 2.x and earlier, and projects using `packages.config`, the `<files>` element is also used to include immutable content files when a package is installed.</span></span> <span data-ttu-id="3e0a0-398">NuGet 3.3 이상 및 프로젝트 PackageReference에서는 `<contentFiles>` 요소가 대신 사용됩니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-398">With NuGet 3.3+ and projects PackageReference, the `<contentFiles>` element is used instead.</span></span> <span data-ttu-id="3e0a0-399">자세한 내용은 아래의 [콘텐츠 파일 포함](#including-content-files)을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-399">See [Including content files](#including-content-files) below for details.</span></span>

### <a name="file-element-attributes"></a><span data-ttu-id="3e0a0-400">파일 요소 특성</span><span class="sxs-lookup"><span data-stu-id="3e0a0-400">File element attributes</span></span>

<span data-ttu-id="3e0a0-401">각 `<file>` 요소는 다음과 같은 특성을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-401">Each `<file>` element specifies the following attributes:</span></span>

| <span data-ttu-id="3e0a0-402">attribute</span><span class="sxs-lookup"><span data-stu-id="3e0a0-402">Attribute</span></span> | <span data-ttu-id="3e0a0-403">설명</span><span class="sxs-lookup"><span data-stu-id="3e0a0-403">Description</span></span> |
| --- | --- |
| <span data-ttu-id="3e0a0-404">**src**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-404">**src**</span></span> | <span data-ttu-id="3e0a0-405">`exclude` 특성으로 지정된 제외에 따라 포함할 파일의 위치입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-405">The location of the file or files to include, subject to exclusions specified by the `exclude` attribute.</span></span> <span data-ttu-id="3e0a0-406">절대 경로가 지정되지 않으면 경로는 `.nuspec` 파일에 대한 상대 경로입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-406">The path is relative to the `.nuspec` file unless an absolute path is specified.</span></span> <span data-ttu-id="3e0a0-407">`*` 와일드카드 문자가 허용되고, `**` 이중 와일드카드는 재귀적 폴더 검색을 의미합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-407">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="3e0a0-408">**대상**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-408">**target**</span></span> | <span data-ttu-id="3e0a0-409">원본 파일이 있는 패키지 내의 폴더에 대한 상대 경로이며, `lib`, `content`, `build` 또는 `tools`로 시작해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-409">The relative path to the folder within the package where the source files are placed, which must begin with `lib`, `content`, `build`, or `tools`.</span></span> <span data-ttu-id="3e0a0-410">[규칙 기반 작업 디렉터리에서 .nuspec 만들기](../create-packages/creating-a-package.md#from-a-convention-based-working-directory)를 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-410">See [Creating a .nuspec from a convention-based working directory](../create-packages/creating-a-package.md#from-a-convention-based-working-directory).</span></span> |
| <span data-ttu-id="3e0a0-411">**시키고**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-411">**exclude**</span></span> | <span data-ttu-id="3e0a0-412">`src` 위치에서 제외할 파일 또는 파일 패턴에 대한 세미콜론으로 구분된 목록입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-412">A semicolon-delimited list of files or file patterns to exclude from the `src` location.</span></span> <span data-ttu-id="3e0a0-413">`*` 와일드카드 문자가 허용되고, `**` 이중 와일드카드는 재귀적 폴더 검색을 의미합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-413">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |

### <a name="examples"></a><span data-ttu-id="3e0a0-414">예</span><span class="sxs-lookup"><span data-stu-id="3e0a0-414">Examples</span></span>

<span data-ttu-id="3e0a0-415">**단일 어셈블리**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-415">**Single assembly**</span></span>
...
<span data-ttu-id="3e0a0-416">**특정 대상 프레임워크에 대한 단일 어셈블리**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-416">**Single assembly specific to a target framework**</span></span>
...
<span data-ttu-id="3e0a0-417">**와일드카드를 사용하는 DLL 집합**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-417">**Set of DLLs using a wildcard**</span></span>
...
<span data-ttu-id="3e0a0-418">**여러 프레임워크에 대한 DLL**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-418">**DLLs for different frameworks**</span></span>
...
<span data-ttu-id="3e0a0-419">**파일 제외**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-419">**Excluding files**</span></span>
...
## <a name="including-content-files"></a><span data-ttu-id="3e0a0-420">콘텐츠 파일 포함</span><span class="sxs-lookup"><span data-stu-id="3e0a0-420">Including content files</span></span>

<span data-ttu-id="3e0a0-421">콘텐츠 파일은 패키지에서 프로젝트에 포함해야 하는 변경할 수 없는 파일입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-421">Content files are immutable files that a package needs to include in a project.</span></span> <span data-ttu-id="3e0a0-422">변경할 수 없으므로 사용하는 프로젝트에서 수정되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-422">Being immutable, they are not intended to be modified by the consuming project.</span></span> <span data-ttu-id="3e0a0-423">콘텐츠 파일의 예는 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-423">Example content files include:</span></span>

- <span data-ttu-id="3e0a0-424">리소스로 포함된 이미지</span><span class="sxs-lookup"><span data-stu-id="3e0a0-424">Images that are embedded as resources</span></span>
- <span data-ttu-id="3e0a0-425">이미 컴파일된 원본 파일</span><span class="sxs-lookup"><span data-stu-id="3e0a0-425">Source files that are already compiled</span></span>
- <span data-ttu-id="3e0a0-426">프로젝트의 빌드 출력에 포함되어야 하는 스크립트</span><span class="sxs-lookup"><span data-stu-id="3e0a0-426">Scripts that need to be included with the build output of the project</span></span>
- <span data-ttu-id="3e0a0-427">프로젝트에 포함되어야 하지만 프로젝트별 변경이 필요하지 않은 패키지에 대한 구성 파일</span><span class="sxs-lookup"><span data-stu-id="3e0a0-427">Configuration files for the package that need to be included in the project but don't need any project-specific changes</span></span>

<span data-ttu-id="3e0a0-428">콘텐츠 파일은 `<files>` 요소를 사용하고 `target` 특성에서 `content` 폴더를 지정하여 패키지에 포함됩니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-428">Content files are included in a package using the `<files>` element, specifying the `content` folder in the `target` attribute.</span></span> <span data-ttu-id="3e0a0-429">그러나 `<contentFiles>` 요소를 대신 사용하는 PackageReference를 사용하여 프로젝트에 패키지를 설치하는 경우 이러한 파일은 무시됩니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-429">However, such files are ignored when the package is installed in a project using PackageReference, which instead uses the `<contentFiles>` element.</span></span>

<span data-ttu-id="3e0a0-430">사용하는 프로젝트와의 호환성을 최대화하기 위해 패키지는 두 요소 모두에 콘텐츠 파일을 이상적으로 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-430">For maximum compatibility with consuming projects, a package ideally specifies the content files in both elements.</span></span>

### <a name="using-the-files-element-for-content-files"></a><span data-ttu-id="3e0a0-431">콘텐츠 파일에 대한 files 요소 사용</span><span class="sxs-lookup"><span data-stu-id="3e0a0-431">Using the files element for content files</span></span>

<span data-ttu-id="3e0a0-432">콘텐츠 파일의 경우 단순히 어셈블리 파일과 동일한 형식을 사용하고 다음 예제와 같이 `content`를 `target` 특성의 기본 폴더로 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-432">For content files, simply use the same format as for assembly files, but specify `content` as the base folder in the `target` attribute as shown in the following examples.</span></span>

<span data-ttu-id="3e0a0-433">**기본 콘텐츠 파일**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-433">**Basic content files**</span></span>
...
<span data-ttu-id="3e0a0-434">**디렉터리 구조가 포함된 콘텐츠 파일**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-434">**Content files with directory structure**</span></span>
...
<span data-ttu-id="3e0a0-435">**특정 대상 프레임워크에 대한 콘텐츠 파일**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-435">**Content file specific to a target framework**</span></span>
...
<span data-ttu-id="3e0a0-436">**이름에 점이 있는 폴더에 복사되는 콘텐츠 파일**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-436">**Content file copied to a folder with dot in name**</span></span>

<span data-ttu-id="3e0a0-437">이 경우 NuGet은 `target`과 `src`의 확장명이 일치하지 않는지 확인하여 `target`의 이름 중 해당 부분을 폴더로 처리합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-437">In this case, NuGet sees that the extension in `target` does not match the extension in `src` and thus treats that part of the name in `target` as a folder:</span></span>
...
<span data-ttu-id="3e0a0-438">**확장명이 없는 콘텐츠 파일**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-438">**Content files without extensions**</span></span>

<span data-ttu-id="3e0a0-439">확장명이 없는 파일을 포함하려면 `*` 또는 `**` 와일드카드를 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-439">To include files without an extension, use the `*` or `**` wildcards:</span></span>
...
<span data-ttu-id="3e0a0-440">**전체 경로와 전체 대상이 포함된 콘텐츠 파일**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-440">**Content files with deep path and deep target**</span></span>

<span data-ttu-id="3e0a0-441">이 경우 원본과 대상의 파일 확장명이 일치하므로 NuGet은 대상이 폴더가 아니라 파일 이름이라고 가정합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-441">In this case, because the file extensions of the source and target match, NuGet assumes that the target is a file name and not a folder:</span></span>
...
<span data-ttu-id="3e0a0-442">**패키지의 콘텐츠 파일 이름 바꾸기**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-442">**Renaming a content file in the package**</span></span>
...
<span data-ttu-id="3e0a0-443">**파일 제외**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-443">**Excluding files**</span></span>
...
### <a name="using-the-contentfiles-element-for-content-files"></a><span data-ttu-id="3e0a0-444">콘텐츠 파일에 대한 contentFiles 요소 사용</span><span class="sxs-lookup"><span data-stu-id="3e0a0-444">Using the contentFiles element for content files</span></span>

<span data-ttu-id="3e0a0-445">*NuGet 4.0 이상(PackageReference 사용)*</span><span class="sxs-lookup"><span data-stu-id="3e0a0-445">*NuGet 4.0+ with PackageReference*</span></span>

<span data-ttu-id="3e0a0-446">기본적으로 패키지는 `contentFiles` 폴더(아래 참조) 및 기본 특성을 사용하여 해당 폴더의 모든 파일이 포함되는 `nuget pack`에 콘텐츠 파일을 배치합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-446">By default, a package places content in a `contentFiles` folder (see below) and `nuget pack` included all files in that folder using default attributes.</span></span> <span data-ttu-id="3e0a0-447">이 경우 `.nuspec`에 `contentFiles` 노드를 포함할 필요가 없습니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-447">In this case it's not necessary to include a `contentFiles` node in the `.nuspec` at all.</span></span>

<span data-ttu-id="3e0a0-448">포함되는 파일을 제어하려면 `<contentFiles>` 요소에서 포함할 파일을 정확히 식별하는 `<files>` 요소 컬렉션을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-448">To control which files are included, the `<contentFiles>` element specifies is a collection of `<files>` elements that identify the exact files include.</span></span>

<span data-ttu-id="3e0a0-449">이러한 파일은 프로젝트 시스템 내에서 사용되는 방법을 설명하는 일단의 특성으로 지정됩니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-449">These files are specified with a set of attributes that describe how they should be used within the project system:</span></span>

| <span data-ttu-id="3e0a0-450">attribute</span><span class="sxs-lookup"><span data-stu-id="3e0a0-450">Attribute</span></span> | <span data-ttu-id="3e0a0-451">설명</span><span class="sxs-lookup"><span data-stu-id="3e0a0-451">Description</span></span> |
| --- | --- |
| <span data-ttu-id="3e0a0-452">**되어야**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-452">**include**</span></span> | <span data-ttu-id="3e0a0-453">(필수) `exclude` 특성으로 지정된 제외에 따라 포함할 파일의 위치입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-453">(Required) The location of the file or files to include, subject to exclusions specified by the `exclude` attribute.</span></span> <span data-ttu-id="3e0a0-454">`contentFiles`절대 경로를 지정 하지 않으면 폴더에 대 한 상대 경로입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-454">The path is relative to the `contentFiles` folder unless an absolute path is specified.</span></span> <span data-ttu-id="3e0a0-455">`*` 와일드카드 문자가 허용되고, `**` 이중 와일드카드는 재귀적 폴더 검색을 의미합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-455">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="3e0a0-456">**시키고**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-456">**exclude**</span></span> | <span data-ttu-id="3e0a0-457">`src` 위치에서 제외할 파일 또는 파일 패턴에 대한 세미콜론으로 구분된 목록입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-457">A semicolon-delimited list of files or file patterns to exclude from the `src` location.</span></span> <span data-ttu-id="3e0a0-458">`*` 와일드카드 문자가 허용되고, `**` 이중 와일드카드는 재귀적 폴더 검색을 의미합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-458">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="3e0a0-459">**buildAction**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-459">**buildAction**</span></span> | <span data-ttu-id="3e0a0-460">,,, 등의 MSBuild에 대 한 콘텐츠 항목에 할당할 빌드 `Content` 작업 `None` `Embedded Resource` `Compile` 입니다. 기본값은 `Compile` 입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-460">The build action to assign to the content item for MSBuild, such as `Content`, `None`, `Embedded Resource`, `Compile`, etc. The default is `Compile`.</span></span> |
| <span data-ttu-id="3e0a0-461">**copyToOutput**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-461">**copyToOutput**</span></span> | <span data-ttu-id="3e0a0-462">콘텐츠 항목을 빌드 (또는 게시) 출력 폴더에 복사할지 여부를 나타내는 부울 값입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-462">A Boolean indicating whether to copy content items to the build (or publish) output folder.</span></span> <span data-ttu-id="3e0a0-463">기본값은 false입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-463">The default is false.</span></span> |
| <span data-ttu-id="3e0a0-464">**flatten**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-464">**flatten**</span></span> | <span data-ttu-id="3e0a0-465">빌드 출력의 단일 폴더에 콘텐츠 항목을 복사할지(true), 아니면 패키지의 폴더 구조를 유지할지(false) 여부를 나타내는 부울 값입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-465">A Boolean indicating whether to copy content items to a single folder in the build output (true), or to preserve the folder structure in the package (false).</span></span> <span data-ttu-id="3e0a0-466">이 플래그는 copyToOutput 플래그가 true로 설정된 경우에만 작동합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-466">This flag only works when copyToOutput flag is set to true.</span></span> <span data-ttu-id="3e0a0-467">기본값은 false입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-467">The default is false.</span></span> |

<span data-ttu-id="3e0a0-468">패키지를 설치할 때 NuGet에서 `<contentFiles>`의 자식 요소를 위쪽에서 아래쪽으로 적용합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-468">When installing a package, NuGet applies the child elements of `<contentFiles>` from top to bottom.</span></span> <span data-ttu-id="3e0a0-469">여러 항목이 동일한 파일과 일치하면 모든 항목이 적용됩니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-469">If multiple entries match the same file then all entries are applied.</span></span> <span data-ttu-id="3e0a0-470">동일한 특성에 대한 충돌이 있는 경우 최상위 항목이 하위 항목을 재정의합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-470">The top-most entry overrides the lower entries if there is a conflict for the same attribute.</span></span>

#### <a name="package-folder-structure"></a><span data-ttu-id="3e0a0-471">패키지 폴더 구조</span><span class="sxs-lookup"><span data-stu-id="3e0a0-471">Package folder structure</span></span>

<span data-ttu-id="3e0a0-472">패키지 프로젝트는 다음 패턴을 사용하여 콘텐츠를 구성해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-472">The package project should structure content using the following pattern:</span></span>

```
/contentFiles/{codeLanguage}/{TxM}/{any?}
```

- <span data-ttu-id="3e0a0-473">`codeLanguages`는 `cs`, `vb`, `fs`, `any` 또는 지정된 `$(ProjectLanguage)`에 해당하는 소문자일 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-473">`codeLanguages` may be `cs`, `vb`, `fs`, `any`, or the lowercase equivalent of a given `$(ProjectLanguage)`</span></span>
- <span data-ttu-id="3e0a0-474">`TxM`은 NuGet에서 지원하는 모든 법적 대상 프레임워크 모니커입니다([대상 프레임워크](../reference/target-frameworks.md) 참조).</span><span class="sxs-lookup"><span data-stu-id="3e0a0-474">`TxM` is any legal target framework moniker that NuGet supports (see [Target frameworks](../reference/target-frameworks.md)).</span></span>
- <span data-ttu-id="3e0a0-475">모든 폴더 구조는 이 구문의 끝에 추가될 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-475">Any folder structure may be appended to the end of this syntax.</span></span>

<span data-ttu-id="3e0a0-476">예를 들어 다음과 같은 가치를 제공해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-476">For example:</span></span>
...
<span data-ttu-id="3e0a0-477">빈 폴더는 `.`을 사용하여 언어 및 TxM의 특정 조합에 대한 콘텐츠 제공을 거부할 수 있습니다. 예를 들어 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-477">Empty folders can use `.` to opt out of providing content for certain combinations of language and TxM, for example:</span></span>
...
#### <a name="example-contentfiles-section"></a><span data-ttu-id="3e0a0-478">contentFiles 섹션 예제</span><span class="sxs-lookup"><span data-stu-id="3e0a0-478">Example contentFiles section</span></span>
...
## <a name="framework-reference-groups"></a><span data-ttu-id="3e0a0-479">프레임 워크 참조 그룹</span><span class="sxs-lookup"><span data-stu-id="3e0a0-479">Framework reference groups</span></span>

<span data-ttu-id="3e0a0-480">*버전 5.1 + 붙일 PackageReference only*</span><span class="sxs-lookup"><span data-stu-id="3e0a0-480">*Version 5.1+ wih PackageReference only*</span></span>

<span data-ttu-id="3e0a0-481">프레임 워크 참조는 WPF 또는 Windows Forms와 같은 공유 프레임 워크를 나타내는 .NET Core 개념입니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-481">Framework References are a .NET Core concept representing shared frameworks such as WPF or Windows Forms.</span></span>
<span data-ttu-id="3e0a0-482">패키지는 공유 프레임 워크를 지정 하 여 모든 프레임 워크 종속성이 참조 하는 프로젝트에 포함 되도록 합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-482">By specifying a shared framework, the package ensures that all its framework dependencies are included in the referencing project.</span></span>

<span data-ttu-id="3e0a0-483">각 `<group>` 요소에는 `targetFramework` 특성과 0 개 이상의 `<frameworkReference>` 요소가 필요 합니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-483">Each `<group>` element requires a `targetFramework` attribute and zero or more `<frameworkReference>` elements.</span></span>

<span data-ttu-id="3e0a0-484">다음 예제에서는 .NET Core WPF 프로젝트에 대해 생성 되는 nuspec를 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-484">The following example shows a nuspec generated for a .NET Core WPF project.</span></span>
<span data-ttu-id="3e0a0-485">프레임 워크 참조를 포함 하는 nuspecs를 직접 작성 하는 것은 권장 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-485">Note that hand authoring nuspecs that contain framework references is not recommended.</span></span> <span data-ttu-id="3e0a0-486">대신 [대상](msbuild-targets.md) 팩을 사용 하는 것이 좋습니다. 이렇게 하면 프로젝트에서 자동으로 유추 됩니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-486">Consider using the [targets](msbuild-targets.md) pack instead, which will automatically infer them from the project.</span></span>
...
## <a name="example-nuspec-files"></a><span data-ttu-id="3e0a0-487">예제 nuspec 파일</span><span class="sxs-lookup"><span data-stu-id="3e0a0-487">Example nuspec files</span></span>

<span data-ttu-id="3e0a0-488">**dependencies 또는 files를 지정하지 않은 간단한 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-488">**A simple `.nuspec` that does not specify dependencies or files**</span></span>
...
<span data-ttu-id="3e0a0-489">**dependencies가 있는 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-489">**A `.nuspec` with dependencies**</span></span>
...
<span data-ttu-id="3e0a0-490">**files가 있는 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-490">**A `.nuspec` with files**</span></span>
...
<span data-ttu-id="3e0a0-491">**프레임워크 어셈블리가 있는 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="3e0a0-491">**A `.nuspec` with framework assemblies**</span></span>
...
<span data-ttu-id="3e0a0-492">이 예제에서는 특정 프로젝트 대상에 대해 다음이 설치됩니다.</span><span class="sxs-lookup"><span data-stu-id="3e0a0-492">In this example, the following are installed for specific project targets:</span></span>

- <span data-ttu-id="3e0a0-493">.NET4 -> `System.Web`, `System.Net`</span><span class="sxs-lookup"><span data-stu-id="3e0a0-493">.NET4 -> `System.Web`, `System.Net`</span></span>
- <span data-ttu-id="3e0a0-494">.NET4 클라이언트 프로필 -> `System.Net`</span><span class="sxs-lookup"><span data-stu-id="3e0a0-494">.NET4 Client Profile -> `System.Net`</span></span>
- <span data-ttu-id="3e0a0-495">Silverlight 3 -> `System.Json`</span><span class="sxs-lookup"><span data-stu-id="3e0a0-495">Silverlight 3 -> `System.Json`</span></span>
- <span data-ttu-id="3e0a0-496">WindowsPhone -> `Microsoft.Devices.Sensors`</span><span class="sxs-lookup"><span data-stu-id="3e0a0-496">WindowsPhone -> `Microsoft.Devices.Sensors`</span></span>
