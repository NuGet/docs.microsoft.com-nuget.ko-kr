...
<span data-ttu-id="9bce6-117">현재 `nuspec.xsd` 스키마 파일은 [NuGet GitHub 리포지토리](https://github.com/NuGet/NuGet.Client/blob/dev/src/NuGet.Core/NuGet.Packaging/compiler/resources/nuspec.xsd)에 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-117">The current `nuspec.xsd` schema file can be found in the [NuGet GitHub repository](https://github.com/NuGet/NuGet.Client/blob/dev/src/NuGet.Core/NuGet.Packaging/compiler/resources/nuspec.xsd).</span></span>
...
#### <a name="version"></a><span data-ttu-id="9bce6-130">버전</span><span class="sxs-lookup"><span data-stu-id="9bce6-130">version</span></span>
...
<span data-ttu-id="9bce6-148">nuget.org뿐만 아니라 UI 표시에도 종종 표시되는 패키지의 라이선스에 대한 URL입니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-148">A URL for the package's license, often shown in UI displays as well as nuget.org.</span></span>
...
<span data-ttu-id="9bce6-175">*(1.5 이상)* 패키지에 대한 저작권 세부 정보입니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-175">*(1.5+)* Copyright details for the package.</span></span>
...
#### <a name="collection-elements"></a><span data-ttu-id="9bce6-195">컬렉션 요소</span><span class="sxs-lookup"><span data-stu-id="9bce6-195">Collection elements</span></span>
...
<span data-ttu-id="9bce6-257">또한 토큰은 [어셈블리 파일](#including-assembly-files) 및 [콘텐츠 파일](#including-content-files)을 포함할 때 경로를 확인하는 데 사용할 수도 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-257">Tokens can also be used to resolve paths when you include [assembly files](#including-assembly-files) and [content files](#including-content-files).</span></span> <span data-ttu-id="9bce6-258">토큰의 이름은 MSBuild 속성과 동일하므로 현재 빌드 구성에 따라 포함할 파일을 선택할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-258">The tokens have the same names as the MSBuild properties, making it possible to select files to be included depending on the current build configuration.</span></span> <span data-ttu-id="9bce6-259">예를 들어 `.nuspec` 파일에서 다음 토큰을 사용하는 경우가 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-259">For example, if you use the following tokens in the `.nuspec` file:</span></span>
...
| <span data-ttu-id="9bce6-282">런타임(runtime)</span><span class="sxs-lookup"><span data-stu-id="9bce6-282">runtime</span></span> | <span data-ttu-id="9bce6-283">Runtime, Resources 및 FrameworkAssemblies</span><span class="sxs-lookup"><span data-stu-id="9bce6-283">Runtime, Resources, and FrameworkAssemblies</span></span> |
...
<span data-ttu-id="9bce6-310">명시적 참조는 일반적으로 디자인 타임 전용 어셈블리에만 사용됩니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-310">Explicit references are typically used for design-time only assemblies.</span></span> <span data-ttu-id="9bce6-311">예를 들어 [코드 계약](/dotnet/framework/debug-trace-profile/code-contracts)을 사용하는 경우 계약 어셈블리는 확장하는 런타임 어셈블리 옆에 있어야 Visual Studio에서 찾을 수 있습니다. 그러나 계약 어셈블리는 프로젝트에서 참조하거나 프로젝트의 `bin` 폴더에 복사할 필요가 없습니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-311">When using [Code Contracts](/dotnet/framework/debug-trace-profile/code-contracts), for example, contract assemblies need to be next to the runtime assemblies that they augment so that Visual Studio can find them, but the contract assemblies need not be referenced by the project or copied into the project's `bin` folder.</span></span>
...
<span data-ttu-id="9bce6-360">**단일 어셈블리**</span><span class="sxs-lookup"><span data-stu-id="9bce6-360">**Single assembly**</span></span>
...
<span data-ttu-id="9bce6-384">확장명이 없는 파일을 포함하려면 `*` 또는 `**` 와일드카드를 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-384">To include files without an extension, use the `*` or `**` wildcards:</span></span>
...
| <span data-ttu-id="9bce6-401">**exclude**</span><span class="sxs-lookup"><span data-stu-id="9bce6-401">**exclude**</span></span> | <span data-ttu-id="9bce6-402">`src` 위치에서 제외할 파일 또는 파일 패턴에 대한 세미콜론으로 구분된 목록입니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-402">A semicolon-delimited list of files or file patterns to exclude from the `src` location.</span></span> <span data-ttu-id="9bce6-403">`*` 와일드카드 문자가 허용되고, `**` 이중 와일드카드는 재귀적 폴더 검색을 의미합니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-403">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="9bce6-404">**buildAction**</span><span class="sxs-lookup"><span data-stu-id="9bce6-404">**buildAction**</span></span> | <span data-ttu-id="9bce6-405">`Content`, `None`, `Embedded Resource`, `Compile` 등과 같이 MSBuild의 콘텐츠 항목에 할당할 빌드 작업입니다. 기본값은 `Compile`입니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-405">The build action to assign to the content item for MSBuild, such as `Content`, `None`, `Embedded Resource`, `Compile`, etc. The default is `Compile`.</span></span> |
| <span data-ttu-id="9bce6-406">**copyToOutput**</span><span class="sxs-lookup"><span data-stu-id="9bce6-406">**copyToOutput**</span></span> | <span data-ttu-id="9bce6-407">출력 폴더를 빌드에 콘텐츠 항목을 복사 (또는 게시) 여부를 나타내는 부울입니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-407">A Boolean indicating whether to copy content items to the build (or publish) output folder.</span></span> <span data-ttu-id="9bce6-408">기본값은 false입니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-408">The default is false.</span></span> |
| <span data-ttu-id="9bce6-409">**flatten**</span><span class="sxs-lookup"><span data-stu-id="9bce6-409">**flatten**</span></span> | <span data-ttu-id="9bce6-410">빌드 출력의 단일 폴더에 콘텐츠 항목을 복사할지(true), 아니면 패키지의 폴더 구조를 유지할지(false) 여부를 나타내는 부울 값입니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-410">A Boolean indicating whether to copy content items to a single folder in the build output (true), or to preserve the folder structure in the package (false).</span></span> <span data-ttu-id="9bce6-411">이 플래그는 copyToOutput 플래그가 true로 설정된 경우에만 작동합니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-411">This flag only works when copyToOutput flag is set to true.</span></span> <span data-ttu-id="9bce6-412">기본값은 false입니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-412">The default is false.</span></span> |

<span data-ttu-id="9bce6-413">패키지를 설치할 때 NuGet에서 `<contentFiles>`의 자식 요소를 위쪽에서 아래쪽으로 적용합니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-413">When installing a package, NuGet applies the child elements of `<contentFiles>` from top to bottom.</span></span> <span data-ttu-id="9bce6-414">여러 항목이 동일한 파일과 일치하면 모든 항목이 적용됩니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-414">If multiple entries match the same file then all entries are applied.</span></span> <span data-ttu-id="9bce6-415">동일한 특성에 대한 충돌이 있는 경우 최상위 항목이 하위 항목을 재정의합니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-415">The top-most entry overrides the lower entries if there is a conflict for the same attribute.</span></span>

#### <a name="package-folder-structure"></a><span data-ttu-id="9bce6-416">패키지 폴더 구조</span><span class="sxs-lookup"><span data-stu-id="9bce6-416">Package folder structure</span></span>

<span data-ttu-id="9bce6-417">패키지 프로젝트는 다음 패턴을 사용하여 콘텐츠를 구성해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-417">The package project should structure content using the following pattern:</span></span>

    /contentFiles/{codeLanguage}/{TxM}/{any?}

- <span data-ttu-id="9bce6-418">`codeLanguages`는 `cs`, `vb`, `fs`, `any` 또는 지정된 `$(ProjectLanguage)`에 해당하는 소문자일 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-418">`codeLanguages` may be `cs`, `vb`, `fs`, `any`, or the lowercase equivalent of a given `$(ProjectLanguage)`</span></span>
- <span data-ttu-id="9bce6-419">`TxM`은 NuGet에서 지원하는 모든 법적 대상 프레임워크 모니커입니다([대상 프레임워크](../reference/target-frameworks.md) 참조).</span><span class="sxs-lookup"><span data-stu-id="9bce6-419">`TxM` is any legal target framework moniker that NuGet supports (see [Target frameworks](../reference/target-frameworks.md)).</span></span>
- <span data-ttu-id="9bce6-420">모든 폴더 구조는 이 구문의 끝에 추가될 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-420">Any folder structure may be appended to the end of this syntax.</span></span>

<span data-ttu-id="9bce6-421">예를 들어:</span><span class="sxs-lookup"><span data-stu-id="9bce6-421">For example:</span></span>
...
<span data-ttu-id="9bce6-422">빈 폴더는 `.`을 사용하여 언어 및 TxM의 특정 조합에 대한 콘텐츠 제공을 거부할 수 있습니다. 예를 들어 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-422">Empty folders can use `.` to opt out of providing content for certain combinations of language and TxM, for example:</span></span>

    /contentFiles/vb/any/code.vb
    /contentFiles/cs/any/.

#### <a name="example-contentfiles-section"></a><span data-ttu-id="9bce6-423">contentFiles 섹션 예제</span><span class="sxs-lookup"><span data-stu-id="9bce6-423">Example contentFiles section</span></span>
...
## <a name="example-nuspec-files"></a><span data-ttu-id="9bce6-424">예를 들어 nuspec 파일</span><span class="sxs-lookup"><span data-stu-id="9bce6-424">Example nuspec files</span></span>

<span data-ttu-id="9bce6-425">**dependencies 또는 files를 지정하지 않은 간단한 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="9bce6-425">**A simple `.nuspec` that does not specify dependencies or files**</span></span>
...
<span data-ttu-id="9bce6-426">**dependencies가 있는 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="9bce6-426">**A `.nuspec` with dependencies**</span></span>
...
<span data-ttu-id="9bce6-427">**files가 있는 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="9bce6-427">**A `.nuspec` with files**</span></span>
...
<span data-ttu-id="9bce6-428">**프레임워크 어셈블리가 있는 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="9bce6-428">**A `.nuspec` with framework assemblies**</span></span>
...
<span data-ttu-id="9bce6-429">이 예제에서는 특정 프로젝트 대상에 대해 다음이 설치됩니다.</span><span class="sxs-lookup"><span data-stu-id="9bce6-429">In this example, the following are installed for specific project targets:</span></span>

- <span data-ttu-id="9bce6-430">.NET4 -> `System.Web`, `System.Net`</span><span class="sxs-lookup"><span data-stu-id="9bce6-430">.NET4 -> `System.Web`, `System.Net`</span></span>
- <span data-ttu-id="9bce6-431">.NET4 클라이언트 프로필 -> `System.Net`</span><span class="sxs-lookup"><span data-stu-id="9bce6-431">.NET4 Client Profile -> `System.Net`</span></span>
- <span data-ttu-id="9bce6-432">Silverlight 3 -> `System.Json`</span><span class="sxs-lookup"><span data-stu-id="9bce6-432">Silverlight 3 -> `System.Json`</span></span>
- <span data-ttu-id="9bce6-433">Silverlight 4 -> `System.Windows.Controls.DomainServices`</span><span class="sxs-lookup"><span data-stu-id="9bce6-433">Silverlight 4 -> `System.Windows.Controls.DomainServices`</span></span>
- <span data-ttu-id="9bce6-434">WindowsPhone -> `Microsoft.Devices.Sensors`</span><span class="sxs-lookup"><span data-stu-id="9bce6-434">WindowsPhone -> `Microsoft.Devices.Sensors`</span></span>
