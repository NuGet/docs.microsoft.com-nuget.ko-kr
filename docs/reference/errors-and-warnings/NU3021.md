...
NuGet 클라이언트가 `SignedCms` 패키지 시그니처의 타임 스탬프 내에서 개체를 확인 하지 못했습니다.
...
`SignedCms`패키지 시그니처의 타임 스탬프 내에 있는 개체를 확인할 수 없습니다.
...
> [!Note]
> 명령을 실행할 때 `nuget verify -signatures` NU3021가 오류로 발생 합니다. 그렇지 않으면 NU3021가 경고로 발생 합니다.
