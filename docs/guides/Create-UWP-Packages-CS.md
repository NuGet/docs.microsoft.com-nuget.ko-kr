...
1. [`NuGet spec`](/nuget/reference/cli-reference/cli-ref-spec) 명령을 실행하여 `ImageEnhancer.nuspec`(`.csroj` 파일의 이름에서 가져온 파일 이름)를 생성합니다.
...
완료된 `.nuspec`에서 패키지에 포함되어야 하는 모든 파일을 참조하면 [`nuget pack`](/nuget/reference/cli-reference/cli-ref-pack) 명령을 실행할 준비가 되었습니다.
