...
볼 수 있듯이 이식 가능한 라이브러리의 폴더 이름 규칙 패턴을 따르는 '휴대용-{framework 1} + {프레임 워크 n을 (를)'를 프레임 워크 식별자에 따라 기존 [프레임 워크 이름 및 버전 규칙](../schema/target-frameworks.md)합니다. 이름 및 버전 규칙을 준수 하는 한 가지 예외는 Windows Phone 사용 되는 프레임 워크 식별자에 있습니다.  이 모니커는 프레임 워크 이름 'wp' (wp7, wp71 또는 w p 8)를 사용 해야 합니다. ' Silverlight-wp7'를 사용 하 여 예를 들어는 오류가 발생 합니다.
...
|----------------|-----------|
|winRT45 합니다. NETCore45|Windows, windows 8, win, win8|
...
|Windows Phone 8|(지원 되지 않음)|WindowsPhone8 w p 8|
<br/>
